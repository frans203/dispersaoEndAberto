...
static void RedimensionaCuco(tTabelaCuco *tabela, tCEP_Ind chaveEIndice)
{
   ...
   int         i,
               teste, /* Resultado de uma inser��o */
               novoTamanho; /* Tamanho da nova tabela */

      /* O novo tamanho ser� 50% maior do que o antigo valor */
   novoTamanho = 1.5*tabela->tam;

      /* Cria uma nova tabela de dispers�o cuco */
   ...
   for (i = 0; i < tabela->tam; ++i) {
         /* Verifica se a posi��o corrente da primeira tabela cont�m uma chave */
      if (tabela->tab1[i].status == OCUPADO) {
            /* Existe uma chave nesta posi��o */
         teste = InsereCuco( &novaTabela, tabela->tab1[i].chaveEIndice );
            /* Esta inser��o n�o pode falhar */
         ASSEGURA( teste, "Falha de insercao em redimensionamento" );
      }

         /* Verifica se a posi��o corrente da segunda tabela cont�m uma chave */
...
{
   int pos; /* Essa vari�vel n�o � estritamente necess�ria, mas */
            /* ela facilita a escrita das express�es seguintes  */
   ...
   /* A chave ainda n�o foi encontrada. Verifica */
   ...
   return -1; /* A chave n�o foi encontrada em nenhuma das tabelas */
}
...
int RemoveCuco(tTabelaCuco *tabela, tCEP chave)
{
   ...
   pos = tabela->fD1(chave, tabela->tam);
      ...
      /* Tenta efetuar a remo��o na primeira tabela */
   if ( tabela->tab1[pos].status == OCUPADO &&
        !strcmp(tabela->tab1[pos].chaveEIndice.chave, chave) ) {
   ...
   /*                                                                    */
   /* A chave ainda n�o foi removida. Tenta remov�-la da segunda tabela. */
   /*                                                                    */
